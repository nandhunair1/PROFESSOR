...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("ഏതു മൂവി ആണോ വേണ്ടത് അതു Spelling തെറ്റാതെ ഗ്രൂപ്പിൽ ചോദിച്ചാൽ മാത്രമേ ലഭിക്കുകയുള്ളു...!!/n/nWhich movie you want, you can get it only if you ask in the group without making a mistake in spelling...!!‼️/n/n𝗗𝗼𝗻𝘁 𝗮𝗱𝗱 𝘄𝗼𝗿𝗱𝘀 & 𝘀𝘆𝗺𝗯𝗼𝗹𝘀 𝗹𝗶𝗸𝗲 , . -  send,link movie,series,𝗲𝘁𝗰‼️/n/nOtt release ആവുന്നതിനുമുന്നെ മൂവീസ്, സീരീസ് ചൊതിക്കരുത് 😢/n@tvseriezzz_updates ജോയിൻ ചെയ്"Dont ask for movies and series before Ott release 😢/nJoin @tvseriezzz_updates")
